...
\section{Using pandoc}\label{using-pandoc}
...
\section{Specifying formats}\label{specifying-formats}
...
{[}\texttt{multirow}{]} (if the document contains a table with cells
that cross multiple rows),
...
\item
  \texttt{icml}
  (\href{https://manualzz.com/doc/9627253/adobe-indesign-cs6-idml-cookbook}{InDesign
  ICML})
\item
  \texttt{ipynb}
  (\href{https://nbformat.readthedocs.io/en/latest/}{Jupyter notebook})
...
found, pandoc will search for it in the \texttt{templates} subdirectory
of the user data directory (see \texttt{-\/-data-dir}). If no extension
is specified and an extensionless template is not found, pandoc will
look for a template with an extension corresponding to the writer, so
...
specified, the key will be given the value \texttt{true}. Structured
values (lists, maps) cannot be assigned using this option, but they can
be assigned in the \texttt{variables} section of a
\hyperref[defaults-files]{defaults file}.
...
(see \hyperref[heading-identifiers]{Heading identifiers}, below). This
...
\item[\phantomsection\label{option--reference-doc}{\texttt{-\/-reference-doc=}\emph{FILE}\textbar{}\emph{URL}}]
...
Note: if your target format is \texttt{markdown}, \texttt{org}, or
\texttt{typst}, you will need to disable the \texttt{citations}
extension (e.g., \texttt{-t\ markdown-citations}) to see the rendered
citations and bibliography. Otherwise the format's own citation syntax
will be used.
...
\chapter{Defaults files}\label{defaults-files}
...
\section{General options}\label{general-options-1}
...
\section{Reader options}\label{reader-options-1}
...
\section{General writer options}\label{general-writer-options-1}
...
\section{Citation rendering}\label{citation-rendering-1}
...
of \texttt{filters} (see \hyperref[reader-options-1]{Reader options}).

\section{Math rendering in HTML}\label{math-rendering-in-html-1}
...
\section{Template syntax}\label{template-syntax}
...
\subsection{Interpolated variables}\label{interpolated-variables}
...
\subsection{Breakable spaces}\label{breakable-spaces}
...
\subsection{Variables for HTML}\label{variables-for-html}
...
\subsection{Variables for HTML math}\label{variables-for-html-math}

\begin{description}
\tightlist
\item[\texttt{classoption}]
when using \texttt{-\/-katex}, you can render display math equations
flush left using \hyperref[layout]{YAML metadata} or with
...
\subsection{Variables for PowerPoint}\label{variables-for-powerpoint}
...
\subsubsection{Front matter}\label{front-matter}
...
\subsubsection{BibLaTeX Bibliographies}\label{biblatex-bibliographies}
...
\subsection{Variables for Texinfo}\label{variables-for-texinfo}
...
\subsection{Variables for Typst}\label{variables-for-typst}
...
\subsection{\texorpdfstring{Extension:
...
\section{Headings and sections}\label{headings-and-sections}
...
\section{Math Input}\label{math-input}

The extensions
\hyperref[extension-tex_math_dollars]{\texttt{tex\_math\_dollars}},
\hyperref[extension-tex_math_gfm]{\texttt{tex\_math\_gfm}},
...
\section{Raw HTML/TeX}\label{raw-htmltex}
...
This extension can be used with \hyperref[custom-styles]{docx custom
styles}.
...
\subsection{\texorpdfstring{Extension: \texttt{citations}
...
\subsection{\texorpdfstring{Extension: \texttt{fancy\_lists}
...
\chapter{Pandoc's Markdown}\label{pandocs-markdown}
...
\subsection{Setext-style headings}\label{setext-style-headings}
...
\subsection{ATX-style headings}\label{atx-style-headings}
...
\subsection{Heading identifiers}\label{heading-identifiers}
...
\subsection{Fenced code blocks}\label{fenced-code-blocks}
...
\subsection{Block content in list
...
\hyperref[extension-compact_definition_lists]{\texttt{compact\_definition\_lists}
extension}.)
...
\subsection{\texorpdfstring{Extension: \texttt{superscript},
...
\subsection{Generic raw attribute}\label{generic-raw-attribute}
...
\section{Divs and Spans}\label{divs-and-spans}
...
\hyperref[extension-fenced_code_attributes]{Extension:
...
\section{Non-default extensions}\label{non-default-extensions}
...
\item
  ...
  \hyperref[citation-syntax]{Citation syntax});
\item
...
\section{Specifying bibliographic
...
\subsection{Capitalization in titles}\label{capitalization-in-titles}
...
\subsection{Conference Papers, Published
...
\section{Specifying a citation style}\label{specifying-a-citation-style}
...
\section{Citations in note styles}\label{citations-in-note-styles}
...
\section{Including uncited items in the
...
\section{Other relevant metadata
...
\chapter{Slide shows}\label{slide-shows}
...
\section{Structuring the slide show}\label{structuring-the-slide-show}
...
\subsection{PowerPoint layout choice}\label{powerpoint-layout-choice}
...
\subsection{Additional columns attributes in
...
\section{Frame attributes in beamer}\label{frame-attributes-in-beamer}
...
\section{Background in reveal.js, beamer, and
...
\subsection{On all slides (beamer, reveal.js,
...
images have been set on the \hyperref[powerpoint-layout-choice]{relevant
layouts}.

\subsubsection{\texorpdfstring{\texttt{parallaxBackgroundImage}
...
\subsection{On individual slides (reveal.js,
...
\subsection{On the title slide (reveal.js,
...
set on the ``Title Slide'' layout.

\subsection{Example (reveal.js)}\label{example-reveal.js}
...
\chapter{EPUBs}\label{epubs}

\section{EPUB Metadata}\label{epub-metadata}
...
an example of a YAML metadata block with EPUB metadata:
...
\section{\texorpdfstring{The \texttt{epub:type}
...
\section{Linked media}\label{linked-media}
...
\section{EPUB styling}\label{epub-styling}
...
\chapter{Chunked HTML}\label{chunked-html}
...
\hyperref[extension-raw_attribute]{raw attribute}, as shown in the last
...
\chapter{Syntax highlighting}\label{syntax-highlighting}
...
\chapter{Custom Styles}\label{custom-styles}
...
\hyperref[option--reference-doc]{reference doc} file. However, if you
...
using \texttt{div}s and \texttt{span}s, respectively.
...
your \hyperref[option--reference-doc]{reference doc}. If they are
...
\texttt{custom-style} class. A \texttt{custom-style} attribute will be
added for each style. Divs will be created to hold the paragraph styles,
and Spans to hold the character styles. Table styles will be applied
...
\chapter{Custom readers and writers}\label{custom-readers-and-writers}
...
\chapter{Reproducible builds}\label{reproducible-builds}
...
\chapter{Accessible PDFs and PDF archiving
...
\hyperref[extension--tagging]{\texttt{tagging}} format extension to
force markup that is optimized for tagging. This can be combined with
the \texttt{pdfa} variable to generate standard-compliant PDFs. E.g.:
...
\section{WeasyPrint}\label{weasyprint}
...
\section{Prince XML}\label{prince-xml}
...
\chapter{Running pandoc as a web
...
\chapter{Running pandoc as a Lua
...
\texttt{lua} executable}, version 5.4. For full documentation, see the
\href{https://github.com/jgm/pandoc/blob/master/doc/pandoc-lua.md}{pandoc-lua}
man page.

\chapter{A note on security}\label{a-note-on-security}
